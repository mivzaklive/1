...
final class MAIE_Generator
{
    public static function run_job(int $job_id): void
    {
        ...
        // שער מקורות: דורש לפחות 2 מקורות עצמאיים
        $source_count = count($topic['source_notes'] ?? []);
        if ($source_count < 2) {
            self::skip_job($job_id, 'insufficient_sources', "הנושא נפסל: נמצא {$source_count} מקור בלבד. נדרשים לפחות 2 מקורות עצמאיים לפני בחירת הנושא.", $payload);
            return;
        }
    ...
    private static function research_topic(MAIE_OpenAI $client, array $profile): array
    {
        ...
        $web_search_instruction = $use_web_search
            ? "בצע עכשיו חיפוש אינטרנט מקיף. חפש ידיעות וכתבות חדשות שפורסמו ב-{$window} השעות האחרונות בלבד על נושאי הקטגוריה. סרוק אתרי חדשות ישראלים ובינלאומיים, כולל אתרים מקומיים הרלוונטיים לקטגוריה (אתרים איראניים, ערביים, אמריקאיים וכו' בהתאם לנושא). ודא שהכתבה שבחרת פורסמה לראשונה לפני פחות מ-{$window} שעות. ודא שנמצאו לפחות 2 מקורות עצמאיים שמדווחים על אותה התפתחות. אם מצאת כיסוי עדכני מ-{$window} השעות האחרונות ב-2 מקורות לפחות — בחר את הנושא הטוב ביותר. אם כל הכיסוי שמצאת ישן מ-{$window} שעות — החזר דלג."
...
2. עבור חדשות: הידיעה הספציפית שאתה בוחר חייבת להיות פרסום ראשוני שיצא ב-{$window} השעות האחרונות. אסור לבחור ידיעה ישנה, ניתוח שפורסם מחדש, או כתבת רקע שאין בה חדש ממשי. עבור סיפורים מתמשכים — מותר לבחור רק פיתוח ספציפי חדש שפורסם בתוך חלון הזמן.
...
4. חובה שהנושא שתבחר יהיה מכוסה בלפחות 2 מקורות עצמאיים. הוסף את שמות המקורות ל-source_notes.
5. אם אין ודאות סבירה לגבי טריות האירוע, דלג.
6. החזר חבילת עובדות מסודרת ומאומתת בלבד. אל תכתוב עדיין כתבה מלאה.
PROMPT;
