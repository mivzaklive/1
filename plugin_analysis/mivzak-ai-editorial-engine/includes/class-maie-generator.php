...
final class MAIE_Generator
{
    ...
    private static function research_topic(MAIE_OpenAI $client, array $profile): array
    {
        ...
        $web_search_instruction = $use_web_search
            ? 'בצע עכשיו חיפוש אינטרנט מקיף ועדכני על נושאי הקטגוריה. סרוק אתרי חדשות ישראלים ובינלאומיים, כולל אתרים מקומיים הרלוונטיים לקטגוריה (למשל: אתרים איראניים, ערביים, אמריקאיים וכו\' בהתאם לנושא). אם מצאת תוצאות רלוונטיות — בחר את הנושא הטוב ביותר שמצאת. אסור להחזיר דלג אם קיים כיסוי עדכני. החזר דלג רק אם הרשת לא הניבה שום תוצאה רלוונטית לקטגוריה לחלוטין.'
