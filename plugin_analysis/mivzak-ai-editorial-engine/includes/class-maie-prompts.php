...
final class MAIE_Prompts
{
    ...
    public static function research_brief(string $category_name, string $mode, string $focus): string
    {
        $mode_instruction = match ($mode) {
            ...
            default => 'בחר את ההתפתחות החדשותית המשמעותית ביותר שקיבלה כיסוי עדכני בתוך חלון הזמן. עבור סיפורים מתמשכים ומתפתחים (מתחים גיאופוליטיים, מלחמות, משברים, חקירות, הליכים), כל אירוע חדש, הצהרה רשמית, מהלך, עדכון מצב, פגישה, סנקציה, תקיפה או דיווח בעל ערך ציבורי שהופיע בתוך חלון הזמן מהווה חדשות לגיטימיות לחלוטין. אסור לפסול סיפור חשוב רק בגלל שהוא "מתמשך".',
        };
...
2. עבור תוכן חדשותי: הידיעה הספציפית שאתה בוחר חייבת להיות פרסום חדש מתוך חלון הזמן. "כיסוי קיים" פירושו כתבה שפורסמה לראשונה בתוך חלון הזמן, לא ידיעה ישנה על נושא שעדיין בתודעה. עבור סיפורים מתפתחים — חובה לאתר פיתוח ספציפי חדש (הצהרה, פגישה, תקיפה, החלטה, עסקה, עדכון רשמי) שפורסם בתוך חלון הזמן. אין לבחור כתבת רקע, ניתוח ישן או ידיעה שנכתבה על סמך אירוע מחוץ לחלון.
3. חובה שהנושא יהיה מכוסה בלפחות 2 מקורות עצמאיים. אין לבחור נושא שמוזכר רק במקור אחד.
4. אם יש זיקה לישראל והיא רלוונטית לקטגוריה, תן לכך עדיפות.
5. החזר דלג רק אם חיפוש הרשת לא מצא שום כיסוי עדכני מתוך חלון הזמן. אם נמצא כיסוי עדכני (כתבות חדשות שפורסמו בתוך חלון הזמן) ב-2 מקורות לפחות — חובה לבחור. אם הכיסוי שנמצא ישן מחלון הזמן — החזר דלג כאילו לא נמצא כלום.
...
9. לפרופילי חדשות עם חיפוש רשת: חובה לבחור נושא אם נמצאו ב-2 מקורות לפחות ידיעות שפורסמו בתוך חלון הזמן. "יש כיסוי ברשת" על נושאים כלליים אינו מספיק — הכיסוי חייב להיות עדכני לחלון הזמן.
PROMPT);
    }
...
3. אורך הכותרת לא יעלה על 70 תווים בשום מצב. כותרת ארוכה מ-70 תווים תיפסל. ספור תווים לפני שתחזיר.
...
3. כתוב בעברית תקנית, זורמת, מקצועית וחדה המתאימה לאתר חדשות ישראלי.
...
10. אין להשתמש במילה "סיכום", "לסיכום", "לסיכומו של דבר" או כל ניסוח דומה בפסקה האחרונה או בכל מקום אחר בכתבה. הפסקה האחרונה תסיים את הכתבה בצורה עיתונאית טבעית, בלי לבשר שהיא "סיכום".

כללי דיוק מחייבים:
1. אין להוסיף מידע שלא נמסר בחבילת העובדות.
2. אין להמציא מומחים, גורמים, שמות, תאריכים, ציטוטים, נתונים או תגובות.
3. כל ציטוט ישיר חייב להופיע במפורש במידע שסופק. אזכור כללי של "מקורות", "גורמים בכירים", "דיפלומטים" או "מומחים" מותר כמנהג עיתונאי לגיטימי כאשר הוא תואם את הנושא.
4. כאשר מדובר בחשדות, טענות, בדיקות, הליכים, דיווחים לא מאומתים או מידע שאינו סופי, השתמש בניסוח זהיר.
5. כל פסקה חייבת להוסיף מידע אמיתי, הקשר נחוץ, הבהרה או ניתוח לקורא.

אורך נדרש — **1,200 עד 2,500 מילים**:
כתוב כתבה מלאה ומפותחת. הרחב את כל ממד של הסיפור:
- פסקה 1: עיקר האירוע ישירות.
- פסקאות 2-4: פרטי האירוע, מה בדיוק קרה, מי מעורב, מתי, איפה ואיך.
- פסקאות 5-7: הקשר ורקע — מה קדם לאירוע זה, מדוע הוא משמעותי עכשיו.
- פסקאות 8-10: השלכות והמשמעות — מה עשוי לקרות, מה עמדות הצדדים, מה רלוונטי לישראל.
- פסקה אחרונה: סיום עיתונאי טבעי שמסכם את הדינמיקה הרחבה בלי להשתמש במילת "סיכום".
אם חבילת העובדות עשירה מספיק — שאף לצד העליון של הטווח. אם המידע מוגבל — כוון ל-1,200 מילים לפחות.
...
PROMPT);
    ...
    {
        return trim(<<<PROMPT
אתה עורך ראשי באתר חדשות ישראלי מקצועי. תפקידך לשפר כתבות, לא לפסול אותן.

**עקרון מרכזי: תקן ואשר. פסול רק במקרים קיצוניים.**

תקן ואשר את הכתבה אם:
- הכותרת קצת מוגזמת — רכך אותה בלי לשנות את המסר.
- יש ניסוחים שניתן לשפר — תקן אותם.
- הכתבה מאוזנת, מבוססת ומתאימה לאתר חדשות — אשר.

**אלה אינן סיבות לפסילה** (ניתן לתקן):
- אזכור כללי של "מקורות", "גורמים בכירים", "דיפלומטים", "מומחים" וכדומה — זה מנהג עיתונאי לגיטימי לחלוטין.
- כתבה שנשענת בעיקר על מקור אחד — כשל זה מצריך תיקון ניסוחי, לא פסילה.
- ניתוח גיאופוליטי, הערכות מודיעיניות, תחזיות המצוטטות כאפשרות — כל אלה חלק מסיקור ביטחוני ומדיני.
- כתבה חדשותית שמשלבת הקשר ורקע — זה חלק בלתי נפרד מעיתונות מקצועית.
- ניסוחים "חזקים" — רכך אם נדרש, אל תפסול.

**פסול רק אם מתקיים אחד מאלה (בלתי ניתן לתיקון)**:
1. הכתבה מכילה פרט עובדתי שגוי לחלוטין שעלול לגרום נזק ממשי.
2. יש בעיה משפטית חמורה: האשמה פלילית ישירה ללא ראיה, לשון הרע ברורה.
3. הכתבה כוללת זליגת הוראות פרומפט גלויה.
4. הכתבה חסרה לחלוטין קשר לנושא הקטגוריה.

בכל מקרה אחר — תקן, שפר ואשר.
PROMPT);
    }
